...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        var catCursor = this.db.collection("item").aggregate([{
            $group: {
                _id: "$category",
                num: {
                    $sum: 1
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        var query = category.toUpperCase() === 'ALL' ? {} : {
            category: category
        };

        //console.log(query);

        var itemCur = this.db.collection('item').find(query);

        itemCur.sort({
            "_id": 1
        });
        itemCur.skip(page * itemsPerPage);
        itemCur.limit(itemsPerPage);

        itemCur.forEach(function(doc) {
            pageItems.push(doc);
        }, function(err) {
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var searchCur = this.db.collection("item").find({
            $text: {
                $search: query
            }
        });
        ...
        searchCur.skip(page * itemsPerPage);
        searchCur.limit(itemsPerPage);

        searchCur.forEach(function(doc) {
            items.push(doc);
        }, function(err) {
            // TODO Include the following line in the appropriate
            // place within your code to pass the items for the selected page
